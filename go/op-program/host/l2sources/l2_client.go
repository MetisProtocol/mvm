...
func (s *L2Client) L2BlockRefByLabel(ctx context.Context, label eth.BlockLabel) (eth.L2BlockRef, error) {
	...
	s.l2BlockRefsCache.Add(common.Hash(ref.Hash), ref)
	return ref, nil
}
...
func (s *L2Client) L2BlockRefByHash(ctx context.Context, hash common.Hash) (eth.L2BlockRef, error) {
	...
	envelope, err := s.PayloadByHash(ctx, ethcommon.Hash(hash))
	if err != nil {
		// w%: wrap to preserve ethereum.NotFound case
...
func (s *L2Client) SystemConfigByL2Hash(ctx context.Context, hash common.Hash) (eth.SystemConfig, error) {
	...
	cfg, err := opderive.PayloadToSystemConfig(s.rollupCfg, envelope.ExecutionPayload)
	if err != nil {
...
func (s *L2Client) OutputV0AtBlock(ctx context.Context, blockHash common.Hash) (*eth.OutputV0, error) {
	head, err := s.InfoByHash(ctx, ethcommon.Hash(blockHash))
	if err != nil {
	...
	return &eth.OutputV0{
		...
		// we are not using the message passer storage root as our commitment
		MessagePasserStorageRoot: eth.Bytes32{},
		BlockHash:                ethcommon.Hash(blockHash),
	}, nil
}
