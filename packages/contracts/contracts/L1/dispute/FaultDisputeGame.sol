...
contract FaultDisputeGame is IFaultDisputeGame, Clone, ISemver {
    ...
    constructor(
        ...
        uint256 maxGameDepthExtension = uint256(_clockExtension.raw()) +
            uint256(_vm.oracle().challengePeriod());
    ...
    function initialize() public payable virtual {
        ...
        IMVMStateCommitmentChain stateCommitmentChain = IMVMStateCommitmentChain(
            ADDRESS_MANAGER.getAddress(SCC_NAME)
        );
        (
            IMVMStateCommitmentChain.BatchInfo memory lastFinalized,
            IMVMStateCommitmentChain.BatchInfo memory earliestDisputable
        ) = stateCommitmentChain.findEarliestDisputableBatch(L2_CHAIN_ID);
        ...
        if (stateCommitmentChain.isDisputedBatch(earliestDisputable.batchHeaderHash))
            revert ClaimAlreadyResolved();

        // Set the starting output root.
        startingOutputRoot = OutputRoot({
            l2BlockNumber: lastFinalized.lastL2BlockNumber,
            root: Hash.wrap(lastFinalized.batchHeaderHash)
        });
        ...
        if (l2BlockNumber() < earliestDisputable.lastL2BlockNumber)
    ...
    function challengeRootL2Block(
        ...
        if (Lib_OVMCodec.hashBatchHeader(_outputRootProof) != rootClaim().raw())
            revert InvalidOutputRootProof();

        // Verify the block hash preimage.
        (, , bytes32 latestBlockhash, ) = abi.decode(
            _outputRootProof.extraData,
        ...
        Lib_RLPReader.RLPItem[] memory headerContents = Lib_RLPReader.readList(
            Lib_RLPReader.toRLPItem(_headerRLP)
        );
        bytes memory rawBlockNumber = Lib_RLPReader.readBytes(
        ...
        if (
            blockNumber < _outputRootProof.prevTotalElements + 1 ||
            blockNumber > _outputRootProof.prevTotalElements + _outputRootProof.batchSize
        ) {
    ...
    function resolve() external returns (GameStatus status_) {
        ...
        if (status_ == GameStatus.CHALLENGER_WINS) {
            ...
            IMVMStateCommitmentChain(ADDRESS_MANAGER.getAddress(SCC_NAME)).saveDisputedBatch(
                claimData[0].claim.raw(),
                startingOutputRoot.l2BlockNumber + 1
            );
        }
    }
    ...
    function resolveClaim(uint256 _claimIndex, uint256 _numToResolve) external {
        ...
        if (checkpoint.subgameIndex == challengeIndicesLen) {
            ...
            if (_claimIndex == 0 && l2BlockNumberChallenged) {
                ...
                _distributeBond(
                    countered == address(0) ? subgameRootClaim.claimant : challenger,
                    subgameRootClaim
                );
