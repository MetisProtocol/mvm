  ...
  if index == nil {
    ...
    if s.verifier && s.backend == BackendL1 {
      ...
      if rcfg.DeSeqBlock > 0 && *index+1 >= rcfg.DeSeqBlock {
        // FIXME: This is a temporary fix for the issue that the latest block is not updated in the verifier.
        //        Need to remove this later after testing.
        latestBlock, err := s.client.GetLatestBlock(s.backend)
        if err != nil {
