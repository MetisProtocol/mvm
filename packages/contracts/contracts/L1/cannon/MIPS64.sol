...
} from "./libraries/CannonErrors.sol";
import { MIPS64Arch as arch } from "./libraries/MIPS64Arch.sol";
import { MIPS64Instructions as ins } from "./libraries/MIPS64Instructions.sol";
import { MIPS64Memory } from "./libraries/MIPS64Memory.sol";
import { MIPS64State as st } from "./libraries/MIPS64State.sol";
import { MIPS64Syscalls as sys } from "./libraries/MIPS64Syscalls.sol";
...
contract MIPS64 is ISemver {
    ...
    uint256 internal constant MEM_PROOF_OFFSET =
        THREAD_PROOF_OFFSET + PACKED_THREAD_STATE_SIZE + 32;
    ...
    function handleRMWOps(
        ...
        unchecked {
            ...
            if (_opcode == ins.OP_LOAD_LINKED || _opcode == ins.OP_LOAD_LINKED64) {
                ...
                _opcode == ins.OP_STORE_CONDITIONAL || _opcode == ins.OP_STORE_CONDITIONAL64
            ) {
    ...
    function loadSubWord(
        ...
        uint64 _byteLength,
        bool _signExtend
    ) internal pure returns (uint64 val_) {
    ...
    function handleSyscall(bytes32 _localContext) internal returns (bytes32 out_) {
        unchecked {
            ...
            (uint64 syscall_no, uint64 a0, uint64 a1, uint64 a2) = sys.getSyscallArgs(
                thread.registers
            );
            ...
            if (syscall_no == sys.SYS_MMAP) {
                ...
                if (
                    a0 == sys.CLOCK_GETTIME_REALTIME_FLAG || a0 == sys.CLOCK_GETTIME_MONOTONIC_FLAG
                ) {
                    v0 = 0;
                    v1 = 0;
                    uint64 secs = 0;
                    ...
                    state.memRoot = MIPS64Memory.writeMem(
                        effAddr,
                        MIPS64Memory.memoryProofOffset(MEM_PROOF_OFFSET, 1),
                        secs
                    );
                    ...
                    if (
                        !MIPS64Memory.isValidProof(
                            state.memRoot,
                            effAddr + 8,
                            MIPS64Memory.memoryProofOffset(MEM_PROOF_OFFSET, 2)
                        )
                    ) {
    ...
    function syscallGetRandom(
        ...
        uint64 _a0,
        uint64 _a1
    ) internal pure returns (uint64 v0_, uint64 v1_, bytes32 memRoot_) {
    ...
    ) internal view returns (uint64 v0_, uint64 v1_) {
        bool memUpdated;
        uint64 memAddr;
        (v0_, v1_, _state.preimageOffset, _state.memRoot, memUpdated, memAddr) = sys.handleSysRead(
            _args
        );
        if (memUpdated) {
    ...
    function preemptThread(
        State memory _state,
        ThreadState memory _thread
    ) internal pure returns (bool changedDirections_) {
        // pop thread from the current stack and push to the other stack
        if (_state.traverseRight) {
            require(
                _state.rightThreadStack != EMPTY_THREAD_ROOT,
                "MIPS64: empty right thread stack"
            );
    ...
    function computeThreadRoot(
        ...
        ThreadState memory _thread
    ) internal pure returns (bytes32 out_) {
    ...
    function setThreadStateFromCalldata(ThreadState memory _thread) internal pure {
        ...
        require(
            s >= (THREAD_PROOF_OFFSET + PACKED_THREAD_STATE_SIZE),
