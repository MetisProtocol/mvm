...
// Libraries
import { MIPS64Memory } from "./MIPS64Memory.sol";
import { MIPS64State as st } from "./MIPS64State.sol";
import { MIPS64Arch as arch } from "./MIPS64Arch.sol";
...
library MIPS64Instructions {
    ...
    function execMipsCoreStepLogic(
        CoreStepLogicParams memory _args
    ) internal pure returns (bytes32 newMemRoot_, bool memUpdated_, uint64 effMemAddr_) {
        unchecked {
            ...
            if (_args.opcode == 2 || _args.opcode == 3) {
                ...
                uint64 target = (_args.cpu.nextPC & signExtend(0xF0000000, 32)) |
                    uint64((_args.insn & 0x03FFFFFF) << 2);
            ...
            if (
                _args.opcode >= 0x20 ||
                _args.opcode == OP_LOAD_DOUBLE_LEFT ||
                _args.opcode == OP_LOAD_DOUBLE_RIGHT
            ) {
                // M[R[rs]+SignExtImm]
    ...
    function executeMipsInstruction(
        ExecuteMipsInstructionParams memory _args
    ...
    function selectSubWord(
        ...
        uint64 _byteLength,
        bool _signExtend
    ) internal pure returns (uint64 retval_) {
        (uint64 dataMask, uint64 bitOffset, uint64 bitLength) = calculateSubWordMaskAndOffset(
            _vaddr,
            _byteLength
        );
    ...
    function updateSubWord(
        ...
        uint64 _byteLength,
        uint64 _value
    ) internal pure returns (uint64 word_) {
    ...
    function calculateSubWordMaskAndOffset(
        uint64 _vaddr,
        uint64 _byteLength
    ) internal pure returns (uint64 dataMask_, uint64 bitOffset_, uint64 bitLength_) {
