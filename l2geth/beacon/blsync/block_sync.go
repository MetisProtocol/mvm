...
func (s *beaconBlockSync) Process(requester request.Requester, events []request.Event) {
	for _, e := range events {
		...
		case request.EvResponse, request.EvFail, request.EvTimeout:
			sid, req, resp := e.RequestInfo()
			blockRoot := common.Hash(req.(sync.ReqBeaconBlock))
			log.Debug("Beacon block event", "type", e.Type.Name, "hash", blockRoot)
			if resp != nil {
		...
		case sync.EvNewHead:
			s.serverHeads[e.Server] = e.Data.(types.HeadInfo).BlockRoot
		case request.EvUnregistered:
			delete(s.serverHeads, e.Server)
		}
	}
	s.updateEventFeed()
