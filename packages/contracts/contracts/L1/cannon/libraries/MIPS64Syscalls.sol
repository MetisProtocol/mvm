...
library MIPS64Syscalls {
    ...
    ) internal pure returns (uint64 sysCallNum_, uint64 a0_, uint64 a1_, uint64 a2_) {
        unchecked {
        ...
        unchecked {
            ...
            // pre-image oracle read
                ...
                assembly {
                    ...
                    // mask of all bytes
                    let suffixMask := sub(
                        shl(mul(sub(sub(WORD_SIZE_BYTES, alignment), datLen), 8), 1),
                        1
                    )
