...
interface RequiredEnvVars {
  ...
  // The minimum size in bytes of any L1 transactions generated by the tx batch submitter.
  MIN_L1_TX_SIZE: number
  // The maximum size in bytes of any L1 transactions generated by the tx batch submitter.
  MAX_L1_TX_SIZE: number
  // The maximum number of L2 transactions that can ever be in a batch.
  MAX_TX_BATCH_COUNT: number
  // The minimum size in bytes of any L1 transactions generated by the state batch submitter.
  MIN_L1_STATE_SIZE: number
  // The maximum size in bytes of any L1 transactions generated by the state batch submitter.
  MAX_L1_STATE_SIZE: number
...
export const run = async () => {
  ...
  const requiredEnvVars: RequiredEnvVars = {
    ...
    MIN_L1_STATE_SIZE: config.uint(
      'min-l1-state-size',
      parseInt(env.MIN_L1_STATE_SIZE, 10)
    ),
    MAX_L1_STATE_SIZE: config.uint(
      'max-l1-state-size',
      parseInt(env.MAX_L1_STATE_SIZE, 10)
    ),
  ...
  if (requiredEnvVars.MIN_L1_STATE_SIZE === 0) {
    logger.error('Must configure a MIN_L1_STATE_SIZE greater than 0')
    process.exit(1)
  }
